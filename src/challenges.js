...
// ============================================================
// CHALLENGE ENGINE v3 — DLC-safe
//
// KEY DESIGN: check and progress can be either:
//   A) A function  — used by built-in challenges
//   B) A descriptor object — used by imported DLC packs (JSON can't hold functions)
//
// Descriptor shapes:
//   check:    { type: 'gte' | 'lte' | 'eq' | 'bool', stat: string, value?: number }
//   progress: { stat: string, target: number, unit: string, format?: string }
//
// The engine evaluates both forms via evalCheck() and evalProgress().
// This means DLC packs are pure data — safe to import, store, and share.
// ============================================================

// ── Runtime evaluators ────────────────────────────────────

export const evalCheck = (check, stats) => {
  try {
    if (!check) return false;
    if (typeof check === 'function') return check(stats);
    // Descriptor object
    const val = stats[check.stat];
    switch (check.type) {
      case 'gte':  return Number(val) >= Number(check.value);
      case 'lte':  return Number(val) <= Number(check.value);
      case 'eq':   return String(val) === String(check.value);
...
export const evalProgress = (progress, stats) => {
  try {
    if (!progress) return null;
    if (typeof progress === 'function') return progress(stats);
    // Descriptor object
    const current = stats[progress.stat] ?? 0;
    return {
...
// ── Progress shorthand for built-ins ─────────────────────
...
export const removeDLCPack  = (id)    => storage.set(DLC_KEY, getDLCPacks().filter((p) => p.id !== id));
export const importDLCPack  = (pack)  => {
  const existing = getDLCPacks().filter((p) => p.id !== pack.id);
  storage.set(DLC_KEY, [...existing, pack]);
};

// Flatten DLC packs into typed challenge arrays
// DLC challenges use descriptor objects — evalCheck/evalProgress handle them
...
// ── Lifetime challenge history ────────────────────────────
const HISTORY_KEY = 'challenge_history';

export const getChallengeHistory = () => storage.get(HISTORY_KEY, []);

export const recordChallengeComplete = (id, name, date) => {
  const history = getChallengeHistory();
  if (history.some((h) => h.id === id && h.date === date)) return;
  storage.set(HISTORY_KEY, [
    ...history,
    { id, name, date, completedAt: new Date().toISOString() },
  ]);
};

// ── Seeded picker (same seed = same picks every time) ─────
...
export const getWeekStart  = (d) => {
  ...
  return new Date(new Date(dt).setDate(diff)).toISOString().slice(0, 10);
};
export const getMonthStart = (d) => d.slice(0, 7) + '-01';

// ── Getters ───────────────────────────────────────────────
export const getTodayChallenges = () => {
  const { daily } = getDLCChallenges();
  return seededPick([...DAILY_CHALLENGES, ...daily], 3, 'daily-' + todayISO());
};

export const getWeeklyChallenges = () => {
  const { weekly } = getDLCChallenges();
  return seededPick([...WEEKLY_CHALLENGES, ...weekly], 3, 'weekly-' + getWeekStart(todayISO()));
};

export const getMonthlyChallenges = () => {
  const { monthly } = getDLCChallenges();
  return seededPick([...MONTHLY_CHALLENGES, ...monthly], 3, 'monthly-' + getMonthStart(todayISO()));
};

export const getActiveEventChallenges = () => {
  const { events } = getDLCChallenges();
  const today = todayISO();
  return [...BUILTIN_EVENTS, ...events].filter((e) =>
    !e.period || (today >= e.period.start && today <= e.period.end)
  );
};

// ── Stats object (all numbers, safe for evalCheck) ────────
export const computeChallengeStats = (workouts, weights, supplements, trtLogs, profile) => {
  ...
  let currentStreak = 0;
  if (dates[0] && (dates[0] === today || Math.round((new Date(today + 'T00:00:00') - new Date(dates[0] + 'T00:00:00')) / 86400000) <= 1)) {
    currentStreak = 1;
    for (let i = 1; i < dates.length; i++) {
      const gap = Math.round((new Date(dates[i - 1] + 'T00:00:00') - new Date(dates[i] + 'T00:00:00')) / 86400000);
      if (gap === 1) currentStreak++; else break;
    }
  }
  ...
  return {
    ...
    monthCals, monthMiles, monthWorkouts, monthCardio, monthStrength,
    monthSuppDays, monthWeightLogs, monthWeightLoss,
    sprintToSummerCals, currentStreak,
  };
};
...
export const validateDLCPack = (raw) => {
  ...
  if (!Array.isArray(raw.challenges)) errors.push('Missing challenges array');
  else raw.challenges.forEach((c, i) => {
    ...
    if (c.check && typeof c.check === 'function')
      errors.push(`Challenge ${i}: check must be a descriptor object, not a function`);
    if (c.progress && typeof c.progress === 'function')
      errors.push(`Challenge ${i}: progress must be a descriptor object, not a function`);
  });
  return errors;
};

// ── Sample DLC pack — all descriptor-based, JSON-safe ─────
  ...
  challenges: [
    {
      ...
      desc: 'Burn 500+ calories today.',
      check:    { type: 'gte', stat: 'todayCals',    value: 500 },
      progress: { stat: 'todayCals',    target: 500,   unit: 'cal' },
    },
    {
      id: 'dlc_hh26_w_grind', name: 'December Grind', icon: '❄️', type: 'weekly',
      desc: 'Work out 4 times this week.',
      check:    { type: 'gte', stat: 'weekWorkouts', value: 4 },
      progress: { stat: 'weekWorkouts', target: 4,    unit: 'workouts' },
    },
    {
      id: 'dlc_hh26_w_cals', name: 'Holiday Burn', icon: '🎄', type: 'weekly',
      desc: 'Burn 2,000+ calories this week.',
      check:    { type: 'gte', stat: 'weekCals',     value: 2000 },
      progress: { stat: 'weekCals',     target: 2000, unit: 'cal' },
    },
    {
      id: 'dlc_hh26_m_grind', name: 'December 20', icon: '📅', type: 'monthly',
      desc: 'Log 20 workouts this month.',
      check:    { type: 'gte', stat: 'monthWorkouts', value: 20 },
      progress: { stat: 'monthWorkouts', target: 20,  unit: 'workouts' },
    },
    {
      id: 'dlc_hh26_event_nye', name: 'New Year Ready', icon: '🎆', type: 'event',
      period: { start: '2026-12-01', end: '2026-12-31' },
      desc: 'Burn 12,000 calories in December. Close the year strong.',
      check:    { type: 'gte', stat: 'monthCals',   value: 12000 },
      progress: { stat: 'monthCals',   target: 12000, unit: 'cal' },
      reward: '🎉 New Year Warrior Badge',
    },
  ],
};
