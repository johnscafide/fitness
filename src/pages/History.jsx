...
export default function History({ workouts, setWorkouts, deleteWorkout, showToast }) {
  ...
  const remove = (id) => {
    ...
    deleteWorkout(id); // async Supabase delete
    showToast('Deleted');
  };

  const exportCSV = () => {
    const rows = filtered.map((w) => ({
      date: w.date, type: w.type, equipment: w.equipment,
      calories: w.calories, minutes: w.minutes,
      miles: w.miles || '', avgHR: w.avgHR || '', maxHR: w.maxHR || '',
      exercises: (w.exercises || []).map((e) => `${e.name}: ${e.sets}x${e.reps}@${e.weight}lb`).join(' | '),
      notes: w.notes || '',
    }));
    const headers = [
      { key: 'date', label: 'Date' }, { key: 'type', label: 'Type' },
      { key: 'equipment', label: 'Equipment' }, { key: 'calories', label: 'Calories' },
      { key: 'minutes', label: 'Minutes' }, { key: 'miles', label: 'Miles' },
      { key: 'avgHR', label: 'Avg HR' }, { key: 'maxHR', label: 'Max HR' },
      { key: 'exercises', label: 'Exercises' }, { key: 'notes', label: 'Notes' },
    ];
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        <div style={{
          ...
          padding: '0 12px', flex: 1, minWidth: 200,
        }}>
      ...
      {filtered.length === 0
        ? <div className="card"><div className="empty">No workouts found</div></div>
        : (
          <div className="table-wrap">
            <table className="table">
              <thead>
                <tr>
                  <th>Date</th><th>Type</th><th>Equipment</th><th>Time</th>
                  <th>Calories</th><th>Miles</th><th>Pace</th><th>Avg HR</th>
                  <th>Details</th><th></th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((w) => (
                  <tr key={w.id}>
                    ...
                    <td><span className={`tag ${w.type}`}>{w.type}</span></td>
                    <td>{w.equipment}</td>
