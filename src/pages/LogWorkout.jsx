...
export default function LogWorkout({ workouts, setWorkouts, syncWorkout, showToast, setPage }) {
  ...
  const save = async () => {
    if (!calories && !minutes) { showToast('Need at least calories or minutes'); return; }

    const base = {
      id: uid(), date, type, equipment,
    ...
    const entry = type === 'cardio'
      ? { ...base, miles: Number(miles) || 0, maxHR: Number(maxHR) || null }
      : { ...base, exercises: exercises.filter((e) => e.name.trim()) };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
        <button className={`btn ${type === 'cardio' ? '' : 'secondary'}`}
          onClick={() => { setType('cardio'); setEquipment('Treadmill'); }}>Cardio</button>
        <button className={`btn ${type === 'strength' ? '' : 'secondary'}`}
          onClick={() => { setType('strength'); setEquipment('Barbell'); }}>Strength</button>
      </div>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
            ...
            {exercises.map((ex, i) => (
              <div key={i} style={{
                ...
                <div className="form-group">
                  ...
                  <input value={ex.name} onChange={(e) => updateExercise(i, 'name', e.target.value)} placeholder="Bench press" />
                </div>
                <div className="form-group">
                  {i === 0 && <label>Sets</label>}
                  <input type="number" value={ex.sets} onChange={(e) => updateExercise(i, 'sets', e.target.value)} placeholder="3" />
                </div>
                <div className="form-group">
                  {i === 0 && <label>Reps</label>}
                  <input type="number" value={ex.reps} onChange={(e) => updateExercise(i, 'reps', e.target.value)} placeholder="10" />
                </div>
                <div className="form-group">
                  {i === 0 && <label>Weight (lb)</label>}
                  <input type="number" value={ex.weight} onChange={(e) => updateExercise(i, 'weight', e.target.value)} placeholder="135" />
                </div>
                <button className="icon-btn danger" onClick={() => removeExercise(i)} disabled={exercises.length === 1}>×</button>
              </div>
            ))}
        ...
        <div className="form-group" style={{ marginTop: 16 }}>
          ...
          <textarea value={notes} onChange={(e) => setNotes(e.target.value)}
            placeholder="How did it feel? Anything to remember?" rows={3} />
        </div>
