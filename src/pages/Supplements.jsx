...
export default function Supplements({ supplements, setSupplements, syncSupplement, deleteSupplement, showToast }) {
  ...
  const toggleDefault = (name) => setSelected((prev) => ({ ...prev, [name]: !prev[name] }));

  const saveSelected = () => {
    const toAdd = DEFAULT_SUPPLEMENTS.filter(
      (s) => selected[s.name] && !todayLogs.some((l) => l.name === s.name)
    );
    if (!toAdd.length) { showToast('Nothing new to log'); return; }
    const newLogs = toAdd.map((s) => ({
      id: uid(), date, name: s.name, dose: s.dose, time: s.time, notes: '',
      createdAt: new Date().toISOString(),
    }));
    setSupplements([...supplements, ...newLogs]);
    newLogs.forEach((l) => syncSupplement(l));
    setSelected({});
  ...
  const saveCustom = () => {
    ...
    const entry = {
      id: uid(), date, name: customName.trim(), dose: customDose,
      time: customTime, notes: customNotes, createdAt: new Date().toISOString(),
    };
    setSupplements([...supplements, entry]);
    syncSupplement(entry);
    ...
    <div>
          ...
          <div className="card" style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
              {DEFAULT_SUPPLEMENTS.map((s) => {
                ...
                return (
                  <div key={s.name} onClick={() => !logged && toggleDefault(s.name)}
                    style={{
                      ...
                      cursor: logged ? 'default' : 'pointer', transition: 'all 0.15s',
                    }}>
            ...
            {Object.values(selected).some(Boolean) && (
              <button className="btn" onClick={saveSelected}>Log Selected Supplements</button>
            )}
          </div>
          ...
          <div className="card">
            ...
            <div className="form-grid">
              <div className="form-group">
                <label>Name</label>
                <input value={customName} onChange={(e) => setCustomName(e.target.value)} placeholder="Fish Oil, Zinc..." />
              </div>
              <div className="form-group">
              ...
              <div className="form-group">
                ...
                <select value={customTime} onChange={(e) => setCustomTime(e.target.value)}>
                  {['morning','afternoon','night','pre-workout','post-workout','with meal'].map((t) => (
            ...
            <div className="form-group" style={{ marginTop: 10 }}>
              ...
              <input value={customNotes} onChange={(e) => setCustomNotes(e.target.value)} placeholder="With food, etc." />
            </div>
          ...
          : (
            <div className="table-wrap">
              <table className="table">
                <thead>
                  <tr><th>Date</th><th>Supplement</th><th>Dose</th><th>Time</th><th>Notes</th><th></th></tr>
                </thead>
                <tbody>
      ...
      {view === 'stats' && (
        <div className="grid grid-3">
          ...
          {streaks.length === 0 && <div className="card"><div className="empty">No data yet</div></div>}
        </div>
      )}
    </div>
  );
